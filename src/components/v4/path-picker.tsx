...
export function PathPicker({ onPick }: { onPick: (path: HybridPath) => void }) {
  return (
    <div>
      <h1 className="font-serif text-3xl">Shop your way</h1>
      ...
      <div className="mt-8 grid sm:grid-cols-2 gap-4">
        {PATHS.map(({ key, title, blurb, icon: Icon }) => (
          <button
            key={key}
            onClick={() => onPick(key)}
            className="group text-left rounded-2xl border border-line bg-paper p-6 hover:border-accent hover:shadow-md transition-all"
          >
            <div className="h-10 w-10 rounded-xl bg-accent-soft text-accent flex items-center justify-center mb-4">
