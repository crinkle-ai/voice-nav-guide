...
export const Route = createFileRoute("/v3/priorities")({
  head: () => ({ meta: [{ title: "What matters most — Medicare Compass" }] }),
...
function PrioritiesPage() {
  ...
  useEffect(() => {
    ...
    if (ready && ranked.length === 0) {
      ...
      const seed = Array.from(new Set([...fromIntake]));
      setRanked(seed.slice(0, 3));
    }
  ...
  const cont = () => {
    update({
      ...
      intake: {
        ...state.intake,
        priorities: { value: ranked, confidence: "captured" },
      },
    });
    navigate({ to: "/v3/matches" });
  };
  ...
  return (
    <AppShell step="priorities">
      ...
      <div className="max-w-2xl">
        ...
        <p className="text-muted-2 mt-2">Pick up to three. Drag to rank — #1 is most important.</p>
