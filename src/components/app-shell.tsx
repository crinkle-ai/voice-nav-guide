...
export function AppShell({ children }: { children: ReactNode }) {
  const params = useParams({ strict: false }) as { personaId?: string };
  const personaId = params.personaId ?? "linda";
  const pathname = useRouterState({ select: (s) => s.location.pathname });

  const tabs = [
    { to: "/workspace/$personaId" as const, label: "Workspace", icon: LayoutGrid, match: `/workspace/${personaId}` },
    { to: "/plans/$personaId" as const, label: "Plans", icon: FileText, match: `/plans/${personaId}` },
    { to: "/understanding/$personaId" as const, label: "Shop My Way", icon: UserCog, match: `/understanding/${personaId}` },
  ];

  return (
    <div className="min-h-screen bg-background pb-28">
      {children}
      {/* Floating frosted pill — iOS / Health style */}
